...
set -e
#
# sqlite_fragment_die_test1_tablefile_pageflow_fileparallel_fullscan.sh
#
...
SQLITE_STRICT_COMPILE_FLAGS=${SQLITE_STRICT_COMPILE_FLAGS:-1}
...
DIE_RESULT_BASE="${RESULT_FOLDER%/}/die_test1_tablefile_pageflow_fileparallel_fullscan"

EXE_NAME="sqlite_append_die_affinity_tablefile_pageflow_fileparallel"
SRC_FILE="./$SRC_PATH/sqlite_append_die_affinity_tablefile_pageflow_fileparallel.c"

if [[ ! -f ./${EXE_NAME} ]] || [[ $FORCE_REBUILD == 1 ]]; then
    echo "=== Compiling ${EXE_NAME} (tablefile pageflow file-parallel full-scan test) ==="
    gcc -D_GNU_SOURCE \
...
write_sqlite_run_manifest() {
    ...
    local threads="$6"
    local cold_mode="$7"
    local window_pages="$8"
    local cmt_bytes="$9"
    local stats_file="${RESULT_FOLDER%/}/sqlite_test_phase_stats_${tag}.txt"
    local manifest="${out_dir}/${tag}_manifest.json"
    local args=(
        capture
        --output "$manifest"
        --workload sqlite-tablefile-pageflow
        --variant "$variant"
        ...
        --param "cold_mode=${cold_mode}"
        --param "access_dist=${SQLITE_ACCESS_DIST}"
        --param "target_bytes=${SQLITE_TARGET_BYTES}"
        --param "table_count=${SQLITE_TABLE_COUNT}"
        --param "rows_per_table=${SQLITE_ROWS_PER_TABLE}"
        --param "window_tables=${SQLITE_WINDOW_TABLES}"
        --param "window_pages=${window_pages}"
        --param "window_passes=${SQLITE_WINDOW_PASSES_PER_ROUND}"
        --param "interleave_pages=${interleave_pages}"
        --param "interleave_reads=${SQLITE_INTERLEAVE_READS}"
        --param "cmt_bytes=${cmt_bytes}"
        --param "zipf_seed=${ZIPF_SEED}"
        --param "normal_seed=${NORMAL_SEED}"
        --param "exp_seed=${EXP_SEED}"
        --param "zipf_alpha=${ZIPF_ALPHA}"
        --param "normal_mean=${NORMAL_MEAN}"
        --param "normal_stddev=${NORMAL_STDDEV}"
        --param "cold_extra_mode=${SQLITE_COLD_EXTRA_MODE}"
        --param "cold_extra_append_bytes=${SQLITE_COLD_EXTRA_APPEND_BYTES}"
    )
    ...
    case "$variant" in
        die_latency[123]_norp_sb|die_latency[123]_qlc_all_norp_sb)
            if [[ "$status" == "completed" && "$SQLITE_STRICT_COMPILE_FLAGS" == "1" ]]; then
...
run_one_test() {
    ...
    configure_test_phase_guard_params
    write_sqlite_run_manifest "$variant" "$tag" "$out_dir" \
        "prepared" "$init_txt" "$threads" "$cold_mode" \
        "$window_pages" "$cmt_bytes"
    ...
    if ! write_sqlite_run_manifest "$variant" "$tag" "$out_dir" \
        "completed" "$init_txt" "$threads" "$cold_mode" \
        "$window_pages" "$cmt_bytes"; then
        echo "ERROR: reproducibility manifest or compile contract failed for $tag" >&2
        return 1
    fi
    capture_kernel_tail "$tag" "$init_txt" "$out_dir" "$dmesg_start_line" "$dmesg_start_boot_s"
...
echo ""
    echo "========================================"
echo "  [TEST1-TABLEFILE-PAGEFLOW-FILEPARALLEL-FULLSCAN] All tests completed."
