#!/bin/bash -e
#
# sqlite_fragment_die_test1_chunkpad_fullscan.sh
#
# Die-affinity test 1 (CHUNKPAD + FULL SCAN):
...
DIE_RESULT_BASE="${RESULT_FOLDER%/}/die_test1_natural_chunkpad_fullscan"
...
if [[ ! -f ./${EXE_NAME} ]] || [[ $FORCE_REBUILD == 1 ]]; then
    echo "=== Compiling ${EXE_NAME} (32KB rows, chunkpad full-scan test) ==="
    gcc -D_GNU_SOURCE \
...
run_one_test() {
    local variant="$1"
    local threads="$2"
    local tag="die_natural_chunkpad_fullscan_${variant}_t${threads}"
    local init_txt="${RESULT_FOLDER%/}/sqlite_die_natural_chunkpad_fullscan_init_${variant}_t${threads}.txt"
    ...
    echo ""
    echo "================================================================"
    echo "  [TEST1-NATURAL-CHUNKPAD-FULLSCAN] variant=$variant  threads=$threads  tag=$tag"
...
echo ""
echo "========================================"
echo "  [TEST1-CHUNKPAD-FULLSCAN] All tests completed."
