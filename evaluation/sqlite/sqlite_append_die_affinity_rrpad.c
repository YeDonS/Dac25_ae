...
/*
 * RR-pad mode is experiment-specific: each inserted row is calibrated to one
 * die-sized placement unit, and we want the next row of the same table to
 * advance by exactly one die. With 8 dies, the round-end phase correction is:
 *
 *   pad = (1 - round_size mod 8) mod 8
 *
 * measured in row-sized units.
 */
static unsigned int compute_round_phase_pad_units(unsigned int round_size)
{
	...
	stride = (round_size * RR_PHASE_ROW_UNITS) % RR_PHASE_DIE_COUNT;
...
{
	return (unsigned long long)compute_round_phase_pad_units(round_size) *
	       ROW_PAYLOAD_BYTES;
}
				...
				unsigned int **heat_out)
{
	...
	printf("[sqlite_init] config tables=%u total_rows=%llu row_bytes=%llu row_pages=%u interleave_rows=%u "
	       ...
	       (unsigned int)LPN_PER_ROW, interleave_rows,
	...
	printf("[sqlite_init] rrpad_mode=1 rrpad_die_count=%u row_die_units=%u\n",
	       RR_PHASE_DIE_COUNT, RR_PHASE_ROW_UNITS);
	if (opts->direct_io) {
	...
	if (opts->dummy_interleave) {
		fprintf(stderr,
			"[rrpad] ignoring legacy --dummy-interleave: per-row dummy writes would break table row->die rotation\n");
	}

	dummy_fd = open_dummy_file(TARGET_FOLDER, effective_tag(opts), true);
	if (dummy_fd < 0) {
		fprintf(stderr, "Failed to open rrpad dummy file\n");
		rc = -errno;
		goto out;
	}
	...
	while (rows_written < total_rows) {
		...
		if (round_index >= round_size) {
			...
			if (phase_pad_bytes > 0) {
				...
				if (rc != 0) {
					fprintf(stderr,
						"rrpad phase padding failed: wanted=%llu bytes rc=%d\n",
						phase_pad_bytes, rc);
					goto out;
				}
				...
				if (!need_read_event) {
					...
					if (rc != SQLITE_OK) {
						fprintf(stderr,
							"Failed to restart transaction after rrpad phase padding\n");
						goto out;
					}
					txn_active = true;
				}
