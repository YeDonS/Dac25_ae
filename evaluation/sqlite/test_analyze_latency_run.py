...
def test_cold_read_set_tier_summary_reports_pre_and_post() -> None:
    data = parse_text(
        """
[sqlite_init] tag=sample cold_full_read=1.0s
[sqlite_init] cold_read_set_tier mode=full-scan-all-concurrent phase=pre_cold active_tables=80 file_lpn_slc=100 file_lpn_qlc=20 file_lpn_unknown=0 file_lpn_qlc_pct=16.67 weighted_slc=200 weighted_qlc=60 weighted_unknown=0 weighted_qlc_pct=23.08 tier_entries=1000 tier_parts=1 tier_slc=400 tier_qlc=600 fiemap_files=80 fallback_files=0 fiemap_extents=160 file_lpn_values=120 file_lpn_global_values=120 file_lpn_global_unique=120 file_lpn_cross_file_dups=0 file_lpn_min=1000 file_lpn_max=2000
[sqlite_init] cold_read_set_tier mode=full-scan-all-concurrent phase=post_cold active_tables=80 file_lpn_slc=120 file_lpn_qlc=0 file_lpn_unknown=0 file_lpn_qlc_pct=0.00 weighted_slc=260 weighted_qlc=0 weighted_unknown=0 weighted_qlc_pct=0.00 tier_entries=1000 tier_parts=1 tier_slc=400 tier_qlc=600 fiemap_files=79 fallback_files=1 fiemap_extents=158 file_lpn_values=120 file_lpn_global_values=120 file_lpn_global_unique=30 file_lpn_cross_file_dups=90 file_lpn_min=0 file_lpn_max=2000
"""
    )
    out = StringIO()
    with redirect_stdout(out):
        analyze_latency_run.print_summary(data)
    text = out.getvalue()
    assert "cold_read_set_tier[pre_cold]: mode=full-scan-all-concurrent" in text
    assert "file_qlc=20" in text
    assert "weighted_qlc=60" in text
    assert "weighted_qlc_pct=23.08" in text
    assert "fiemap_files=80 fallback_files=0" in text
    assert "file_global_values=120 file_unique=120 cross_file_dups=0" in text
    assert "cold_read_set_tier[post_cold]: mode=full-scan-all-concurrent" in text
    assert "fiemap_files=79 fallback_files=1" in text
    assert "file_global_values=120 file_unique=30 cross_file_dups=90" in text


def test_active_page_counters_take_precedence_for_qlc_rate() -> None:
    data = parse_text(
        """
[sqlite_init] tag=sample cold_full_read=1.0s
read_requests 1000
host_read_qlc_nand_ops 900
host_read_phys_qlc_nand_ops 800
host_read_qlc_pages 30
host_read_phys_qlc_pages 20
"""
    )
    notes = analyze_latency_run.verdict(data)
    assert "qlc_read_rate=0.030" in notes
    assert analyze_latency_run.host_read_qlc_count(data["stats"]) == 30
    assert analyze_latency_run.host_read_phys_qlc_count(data["stats"]) == 20


def test_test_phase_summary_backfills_page_counters() -> None:
    data = parse_text(
        """
[sqlite_init] tag=sample cold_full_read=1.0s
NVMeVirt: [TEST_PHASE] exit reads=100 overwrites=0 bg_repromote=0 bg_qlc_rebalance=0 read_bg_conflicts=0 read_die_conflicts=0 read_die_wait_ns=0 phys_qlc_reads=2 tier_mismatch_reads=0 phys_qlc_read_pages=64 tier_mismatch_read_pages=0
"""
    )
    stats = data["stats"]
    assert stats["read_requests"] == 100
    assert stats["host_read_phys_qlc_nand_ops"] == 2
    assert stats["host_read_phys_qlc_pages"] == 64
...
"""
    ...
    assert "die_latency3_sb\trandom-row-concurrent+append-concurrent-5.0GiB\tok\thp_lp_completion_priority_active_masked_preheat_unproven\t12.5\t0.500\t1.680\t9.310\t22.000\t19880.000\t1000\t1000\tmissing\tmissing\tmissing\tmissing\t419840\t300\t0\t0\t250\t400\t1.400\t0.025\t0\t0\t0.300\t0\tn/a\tmissing\t5\t50\t0\t0\t10\t8\t2" in text
...
"""
    ...
    mismatch = parse_text(
        """
[sqlite_init] config tables=80 total_rows=128000 interleave_pages=209715 read_ops_per_event=512 init_drop_cache_each_read=0
[sqlite_init] cold_read_latency count=100 avg=0.000200000s p50=0.000030000s p95=0.001000000s p99=0.002000000s p999=0.003000000s max=0.010000000s
[sqlite_init] tag=mismatch tables=80 total_rows=128000 read_events=9 interleaved_read_time=10.0s cold_full_read=10.0s cold_full_read_tp=10.0MB/s cold_mode=random-row-concurrent multifile=1
read_requests 100
"""
    )
    out = StringIO()
    with redirect_stdout(out):
        analyze_latency_run.print_compare([pure, mismatch])
    text = out.getvalue()
    assert "pure\trandom-row-concurrent\tref" in text
    assert "mismatch\trandom-row-concurrent\tmismatch\tnot_comparable(workload_mismatch)" in text
...
def test_request_histogram_is_recomputable() -> None:
    data = parse_text(
        """
...
"""
    ...
    assert analyze_latency_run.read_req_histogram_status(data) == (
        "ok(samples=3,nonzero_bins=2)"
    )


def test_request_histogram_count_mismatch_invalidates_run() -> None:
    data = parse_text(
        """
[kernel_tail_status] status=captured mode=time start_line=1 end_line=2 matches=0
read_req_latency_count 3
read_req_latency_hist_bins 1
read_req_latency_hist_bin_00_upper_ns 1
read_req_latency_hist_bin_00_count 2
"""
    )
    assert analyze_latency_run.read_req_histogram_status(data).startswith("invalid(")
    assert analyze_latency_run.experiment_outcome(data).startswith("invalid_metrics(")
...
def main() -> int:
    ...
    test_cold_read_set_tier_summary_reports_pre_and_post()
    test_active_page_counters_take_precedence_for_qlc_rate()
    test_test_phase_summary_backfills_page_counters()
    ...
    test_request_histogram_is_recomputable()
    test_request_histogram_count_mismatch_invalidates_run()
