...
/*
 * Row size calibration for die-affinity testing.
 *
 * With INTEGER PRIMARY KEY the id is the rowid (not stored in record).
 * Record header: 1 byte (header_size=9) + 4 × 2 bytes (serial type varints) = 9 bytes.
 * Body: 4 × 7281 = 29124 bytes.  Payload P = 29133.
 *
 * SQLite overflow (page_size=4096):
 *   M = ((4096-12)*32/255)-23 = 489,  X = ((4096-12)*64/255)-23 = 1001
 *   P > X → local = M = 489,  remaining = 28644,  overflow = ceil(28644/4092) = 7.
 * Total = 1 leaf + 7 overflow = 8 pages = 32 KB = exactly 1 die (ONESHOT_PAGE_SIZE=32KB,
 * pgs_per_oneshotpg=8).
 */
...
static const struct option long_opts[] = {
	...
	{"chunk-rows", required_argument, NULL, 1034},
...
static void usage(const char *prog)
{
	fprintf(stderr,
		...
		"                 [--interleave-rows N] [--chunk-rows N] [--interleave-reads N]\n"
...
/*
 * Chunk-pad mode: each inserted row is calibrated to one die-sized placement
 * unit, and each table emits chunk_rows consecutive rows before the scheduler
 * advances to the next table. We still phase-correct the end of each round so
 * the next chunk for the same table starts one die ahead.
 */
static unsigned int compute_round_phase_pad_units(unsigned int round_size,
						  unsigned int chunk_rows)
{
	unsigned int stride;

	if (!round_size || !chunk_rows)
		return 0;

	stride = (round_size * chunk_rows) % RR_PHASE_DIE_COUNT;
	return (1U + RR_PHASE_DIE_COUNT - stride) % RR_PHASE_DIE_COUNT;
}

static unsigned long long compute_round_phase_pad_bytes(unsigned int round_size,
							unsigned int chunk_rows)
{
	return (unsigned long long)compute_round_phase_pad_units(round_size, chunk_rows) *
	       ROW_PAYLOAD_BYTES;
}
				...
				unsigned int **heat_out)
{
	...
	unsigned int chunk_rows = opts->chunk_rows ?
	...
	printf("[sqlite_init] config tables=%u total_rows=%llu row_bytes=%llu row_pages=%u interleave_rows=%u "
	       "chunk_rows=%u read_ops_per_event=%u strict_cold_per_select=%u direct_io=%u fast_init_profile=%u "
	       "dummy_interleave=%u dummy_stripe_bytes=%llu\n",
	       table_count, total_rows, (unsigned long long)ROW_PAYLOAD_BYTES,
	       (unsigned int)LPN_PER_ROW, interleave_rows, chunk_rows,
	...
	printf("[sqlite_init] chunkpad_mode=1 rrpad_die_count=%u chunk_rows=%u row_die_units=1\n",
	       RR_PHASE_DIE_COUNT, chunk_rows);
	if (opts->direct_io) {
	...
	if (opts->dummy_interleave) {
		fprintf(stderr,
			"[chunkpad] ignoring legacy --dummy-interleave: per-table chunk rotation owns the phase padding\n");
	}

	dummy_fd = open_dummy_file(TARGET_FOLDER, effective_tag(opts), true);
	if (dummy_fd < 0) {
		fprintf(stderr, "Failed to open chunkpad dummy file\n");
		rc = -errno;
		goto out;
	}
	...
	while (rows_written < total_rows) {
		...
		unsigned int rows_this_step = chunk_rows;
		...
		if (!round_order || round_index >= round_size) {
			...
			memcpy(round_order, active, round_size * sizeof(*round_order));
		}
		...
		need_read_event = ((rows_written && interleave_rows &&
				   (rows_written % interleave_rows == 0)) ||
		...
		if (round_index >= round_size) {
			...
			if (rows_written < total_rows && active_count > 0)
				phase_pad_bytes = compute_round_phase_pad_bytes(round_size,
								chunk_rows);
			...
			if (phase_pad_bytes > 0) {
				...
				if (rc != 0) {
					fprintf(stderr,
						"chunkpad phase padding failed: wanted=%llu bytes rc=%d\n",
						phase_pad_bytes, rc);
					goto out;
				}
				...
				if (!need_read_event) {
					...
					if (rc != SQLITE_OK) {
						fprintf(stderr,
							"Failed to restart transaction after chunkpad phase padding\n");
						goto out;
					}
					txn_active = true;
				}
	...
	printf("[sqlite_init] chunkpad phase_pad_events=%u phase_pad_bytes=%llu\n",
