...
struct file_lpn_collect_stats {
	unsigned int used_fiemap;
	unsigned int used_fallback;
	unsigned int fiemap_extents;
	unsigned long long lpn_min;
	unsigned long long lpn_max;
};
...
static int load_page_tier_entries(const char *path, struct page_tier_entry **entries_out,
				  size_t *count_out, unsigned int *found_parts_out)
{
...
static int collect_file_lpns_with_stats(const char *path,
					unsigned long long host_page_bytes,
					unsigned long long **vec_out,
					size_t *count_out,
					struct file_lpn_collect_stats *stats)
{
	struct file_extent *extents = NULL;
	size_t extent_count = 0;
	...
	*count_out = 0;
	...
	if (rc == 0 && extent_count > 0) {
		...
		if (stats) {
			stats->used_fiemap = 1U;
			stats->fiemap_extents = (unsigned int)extent_count;
		}
		for (size_t i = 0; i < extent_count; ++i) {
...
{
	return collect_file_lpns_with_stats(path, host_page_bytes, vec_out,
					    count_out, NULL);
}
					     ...
					     const char *phase)
{
	...
	unsigned long long tier_slc = 0, tier_qlc = 0, tier_raw_known = 0;
	unsigned long long tier_mismatch = 0;
	unsigned long long file_lpn_values = 0;
	unsigned long long file_lpn_global_values = 0;
	unsigned long long file_lpn_global_unique = 0;
	unsigned long long file_lpn_cross_file_dups = 0;
	unsigned long long file_lpn_min = 0, file_lpn_max = 0;
	unsigned long long *all_lpns = NULL;
	size_t all_count = 0, all_cap = 0;
	unsigned int fiemap_files = 0, fallback_files = 0;
	unsigned int fiemap_extents = 0;
	unsigned int active_tables = 0;
	bool file_lpn_has_global = false;

	if (!opts || !layout || !tables || !tier_entries || tier_count == 0)
		return;

	for (size_t i = 0; i < tier_count; ++i) {
		if (tier_entries[i].in_slc)
			tier_slc++;
		else
			tier_qlc++;
		if (tier_entries[i].raw_known)
			tier_raw_known++;
		if (tier_entries[i].tier_mismatch)
			tier_mismatch++;
	}
	...
	for (unsigned int tbl = 0; tbl < layout->table_count; ++tbl) {
		unsigned long long *lpns = NULL;
		size_t lpn_count = 0;
		struct file_lpn_collect_stats collect_stats;
		unsigned int reads;
		int rc;
		...
		rc = collect_file_lpns_with_stats(tables[tbl].db_path,
						  opts->ftl_host_page_bytes,
						  &lpns, &lpn_count,
						  &collect_stats);
		if (rc != 0)
		...
		if (collect_stats.used_fiemap) {
			fiemap_files++;
			fiemap_extents += collect_stats.fiemap_extents;
		}
		if (collect_stats.used_fallback)
			fallback_files++;
		file_lpn_values += lpn_count;

		if (collect_stats.used_fallback) {
			file_unknown += lpn_count;
			weighted_unknown += (unsigned long long)lpn_count * reads;
			free(lpns);
			continue;
		}

		if (collect_stats.used_fiemap && lpn_count > 0) {
			if (!file_lpn_has_global || collect_stats.lpn_min < file_lpn_min)
				file_lpn_min = collect_stats.lpn_min;
			if (!file_lpn_has_global || collect_stats.lpn_max > file_lpn_max)
				file_lpn_max = collect_stats.lpn_max;
			file_lpn_has_global = true;
		}
		...
		for (size_t i = 0; i < lpn_count; ++i) {
			...
			rc = append_lpn_value(lpns[i], &all_lpns, &all_count, &all_cap);
			if (rc != 0) {
				free(lpns);
				free(all_lpns);
				return;
...

	{
		...
		if (all_count > 1) {
			qsort(all_lpns, all_count, sizeof(*all_lpns), cmp_u64);
			file_lpn_global_unique = 1;
			for (size_t i = 1; i < all_count; ++i) {
				if (all_lpns[i] != all_lpns[file_lpn_global_unique - 1])
					all_lpns[file_lpn_global_unique++] = all_lpns[i];
			}
		} else {
			file_lpn_global_unique = all_count;
		}
		if (file_lpn_global_values >= file_lpn_global_unique)
			file_lpn_cross_file_dups = file_lpn_global_values - file_lpn_global_unique;
		...
		printf("[sqlite_init] cold_read_set_tier mode=%s phase=%s active_tables=%u "
		       ...
		       "weighted_tier_mismatch=%llu tier_entries=%zu tier_parts=%u "
		       "tier_slc=%llu tier_qlc=%llu tier_raw_known=%llu tier_mismatch=%llu "
		       "fiemap_files=%u fallback_files=%u fiemap_extents=%u "
		       "file_lpn_values=%llu file_lpn_global_values=%llu "
		       "file_lpn_global_unique=%llu "
		       "file_lpn_cross_file_dups=%llu file_lpn_min=%llu file_lpn_max=%llu\n",
		       cold_full_read_mode_label(opts->cold_full_read_mode),
		       phase ? phase : "unknown",
		       active_tables,
		       ...
		       tier_count, tier_parts, tier_slc, tier_qlc,
		       tier_raw_known, tier_mismatch,
		       fiemap_files, fallback_files, fiemap_extents,
		       file_lpn_values, file_lpn_global_values, file_lpn_global_unique,
		       file_lpn_cross_file_dups, file_lpn_min, file_lpn_max);
	}
	free(all_lpns);
}
						      ...
						      const char *phase)
{
	...
	print_cold_read_set_tier_summary(opts, layout, tables, read_plan,
					 tier_entries, tier_count, tier_parts, phase);
	free(tier_entries);
}
...
static int run_init_mode(const struct workload_options *opts)
{
	...
	unsigned int page_tier_parts = 0;
	...
	rc = 0;
	rc = load_page_tier_entries(opts->page_tier_path, &page_tier_entries,
				    &page_tier_count, &page_tier_parts);
	if (rc != 0)
