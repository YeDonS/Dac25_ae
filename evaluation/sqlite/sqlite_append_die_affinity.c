...
static int load_page_die_entries(const char *path, struct page_die_entry **entries_out,
				 size_t *count_out, unsigned int *die_count_out)
{
	FILE *fp;
				...
				unsigned long long host_page_bytes)
{
	...
	for (unsigned int tbl = 0; tbl < layout->table_count; ++tbl) {
		...
		for (size_t i = 0; i < lpn_count; ++i) {
			...
			if (!die_known) {
				/* no-op: unknown die is excluded from die histogram */
			}
		}
		...
		fprintf(fp,
			"%u,%s,%llu,%zu,%llu,%llu,%llu,%llu,%llu,%llu,%.9f,%llu,%llu,%llu,%llu,%llu,%.9f,%.9f,%.9f,%u,%d,%llu,%.9f,%.9f,%.9f,%.9f,%.2f\n",
	...
	while (rows_written < total_rows) {
		...
		if (table->rows_inserted >= table->total_rows) {
			...
			if (opts->dummy_interleave) {
				...
				rc = write_dummy_padding(dummy_fd, dummy_buf,
							 opts->dummy_stripe_bytes);
				if (rc != 0) {
					fprintf(stderr,
						"dummy padding failed: wanted=%llu bytes rc=%d\n",
						(unsigned long long)opts->dummy_stripe_bytes, rc);
					goto out;
				}
				...
				if (rc != SQLITE_OK) {
					fprintf(stderr,
						"Failed to restart transaction after dummy padding\n");
					goto out;
				}
				txn_active = true;
				rows_since_commit = 0;
