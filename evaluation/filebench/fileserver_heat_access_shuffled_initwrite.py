...
def create_file_real_write(path, size_bytes):
    fd = os.open(path, os.O_CREAT | os.O_TRUNC | os.O_RDWR, 0o666)
    try:
        chunk_size = min(size_bytes, READ_CHUNK)
        if chunk_size <= 0:
            chunk_size = 4096
        buf = bytes([0x5A]) * chunk_size
        written = 0
        while written < size_bytes:
            todo = min(chunk_size, size_bytes - written)
            ret = os.write(fd, buf[:todo])
            if ret <= 0:
                raise OSError("short write during init write")
            written += ret
        os.fdatasync(fd)
    finally:
        os.close(fd)
...
def create_files(root, file_count, init_size_bytes):
    ...
    for idx in range(file_count):
        ...
        create_file_real_write(path, init_size_bytes)
        paths.append(path)
    os.sync()
    ...
    with open(path, "r", newline="") as fh:
        ...
        for row in reader:
            rows.append(
                {
                    "rank": int(row["rank"]),
                    "base_rank": int(row.get("base_rank", row["rank"])),
                    "path": row["path"],
...
def write_plan_csv(path, rows, weights, counts):
    with open(path, "w", newline="") as fh:
        ...
        writer.writerow(["rank", "base_rank", "path", "weight", "count"])
        for row, weight, count in zip(rows, weights, counts):
            writer.writerow([row["rank"], row["base_rank"], row["path"], f"{weight:.12f}", count])
...
def write_state_csv(path, rows):
    with open(path, "w", newline="") as fh:
        writer = csv.DictWriter(
            fh,
            fieldnames=["rank", "base_rank", "path", "weight", "append_count", "heat", "size_bytes"],
        )
        writer.writeheader()
        for row in rows:
            writer.writerow(row)
...
def print_top_counts(label, rows, counts, topn):
    ...
    for row, count in zip(rows, counts):
        if count > 0:
            ranked.append((count, row["rank"], row["base_rank"], row["path"]))
    ranked.sort(reverse=True)
    for order, item in enumerate(ranked[:topn], start=1):
        count, rank, base_rank, path = item
        print(f"[{label}] top{order}: shuf_rank={rank} base_rank={base_rank} "
...
def phase_create_append(args):
    ...
    print(f"[{args.label}] init_mode=real_write rank_mode="
          f"{'shuffled' if args.shuffle_ranks else 'sorted'}")

    ranked_paths = list(paths)
    if args.shuffle_ranks:
        shuffle_rng = random.Random(args.shuffle_seed)
        shuffle_rng.shuffle(ranked_paths)

    path_to_base_rank = {path: idx for idx, path in enumerate(paths)}
    rows = []
    for idx, (path, weight) in enumerate(zip(ranked_paths, weights)):
        rows.append(
            {
                "rank": idx,
                "base_rank": path_to_base_rank[path],
                "path": path,
                "weight": weight,
                "append_count": 0,
                "heat": 1,
...
def main():
    ...
    ap.add_argument("--shuffle-ranks", type=int, choices=[0, 1], default=1)
    ap.add_argument("--shuffle-seed", type=int, default=20250401)
    ...
    try:
        if args.phase == "create-append":
            args.shuffle_ranks = bool(args.shuffle_ranks)
