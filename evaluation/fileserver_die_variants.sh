...
run_file_tier_analysis() {
    ...
    if grep -q '^files=0$' "$out_summary" 2>/dev/null; then
        echo "WARNING: empty per-file tier analysis for state_tag=$state_tag root=$root" >&2
    fi
}
...
run_one_variant() {
    ...
    if [[ "$ACCESS_DIST" != "none" ]]; then
        ...
        run_file_tier_analysis "$variant" "${variant}_warmup" "warmup_${variant}" "$out_dir" "$analyze_root"
        drop_caches
        sleep 2
        for reads in $(seq 1 "$READ_RUNS"); do
            ...
            run_file_tier_analysis "$variant" "${variant}_read${reads}" "read${reads}_${variant}" "$out_dir" "$analyze_root"
        done
    else
    if [[ "$APPEND_PROFILE" == "hotcold" ]]; then
    ...
    run_file_tier_analysis "$variant" "$variant" "" "$out_dir" "$analyze_root"
