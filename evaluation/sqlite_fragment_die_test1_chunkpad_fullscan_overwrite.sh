#!/bin/bash -e
#
# sqlite_fragment_die_test1_chunkpad_fullscan_overwrite.sh
#
# Die-affinity test 1 (CHUNKPAD + FULL SCAN + OVERWRITE):
#   - init uses sqlite_append_die_affinity_chunkpad.c
#   - each round writes chunk_rows consecutive rows per table before rotating
...
SQLITE_CHUNK_ROWS=${SQLITE_CHUNK_ROWS:-8}
...
DIE_RESULT_BASE="${RESULT_FOLDER%/}/die_test1_natural_chunkpad_fullscan_overwrite"

EXE_NAME="sqlite_append_die_affinity_chunkpad"
SRC_FILE="./$SRC_PATH/sqlite_append_die_affinity_chunkpad.c"

if [[ ! -f ./${EXE_NAME} ]] || [[ $FORCE_REBUILD == 1 ]]; then
    echo "=== Compiling ${EXE_NAME} (32KB rows, chunkpad overwrite full-scan test) ==="
    gcc -D_GNU_SOURCE \
...
run_one_test() {
    local variant="$1"
    local threads="$2"
    local tag="die_natural_chunkpad_fullscan_overwrite_${variant}_t${threads}"
    local init_txt="${RESULT_FOLDER%/}/sqlite_die_natural_chunkpad_fullscan_overwrite_init_${variant}_t${threads}.txt"
    ...
    echo ""
    echo "================================================================"
    echo "  [TEST1-NATURAL-CHUNKPAD-FULLSCAN-OVERWRITE] variant=$variant  threads=$threads  tag=$tag"
    echo "  row_bytes=32KB  tables=$SQLITE_TABLE_COUNT  rows/tbl=$SQLITE_ROWS_PER_TABLE"
    echo "  target=$SQLITE_TARGET_BYTES  phase_pad=ON  chunk_rows=$SQLITE_CHUNK_ROWS  cold_mode=$SQLITE_COLD_FULL_READ_MODE"
...
echo ""
echo "========================================"
echo "  [TEST1-CHUNKPAD-FULLSCAN-OVERWRITE] All tests completed."
