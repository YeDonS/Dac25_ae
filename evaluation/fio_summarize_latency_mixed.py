...
DIST_TAG_PATTERNS = (
    r"zipf(?:_a[A-Za-z0-9_]+)?",
    r"normal(?:_d[A-Za-z0-9_]+)?",
    r"uniform",
    r"random",
)
...
def parse_run_name(path: Path) -> dict[str, str]:
    stem = path.stem
    match = re.match(
        r"^fio_mixed_(?P<body>.+)_rw(?P<ratio>[0-9]+(?:_[0-9]+)?)(?:_bg(?P<bg_timing>[01]))?(?:_r(?P<trial>[0-9]+))?(?:_(?P<ts>[0-9]{8}_[0-9]{6}))?$",
        stem,
    )
    if not match:
        return {
    ...
    dist = ""
    variant = body
    for pattern in DIST_TAG_PATTERNS:
        dist_match = re.search(rf"_(?P<dist>{pattern})$", body)
        if dist_match:
            dist = dist_match.group("dist")
            variant = body[: dist_match.start()]
            break
    ...
    return {
        ...
        "trial": match.group("trial") or "",
        "bg_timing": match.group("bg_timing") or "",
    }
...
def collect_json_files(paths: list[Path]) -> list[Path]:
    ...
    for path in paths:
        if path.is_dir():
            files.extend(
                candidate for candidate in sorted(path.rglob("fio_mixed_*.json"))
                if not candidate.name.endswith("_manifest.json")
            )
        elif (path.is_file() and path.suffix == ".json" and
              not path.name.endswith("_manifest.json")):
            files.append(path)
        else:
...
def summarize_one(json_path: Path, stats_index: dict[str, Path]) -> dict[str, str]:
    ...
    row["fg_read_die_bg_wait_ms_per_read"] = ratio(fg.get("read_die_bg_wait_ns", 0) / 1_000_000.0, fg_reads)
    row["fg_read_ch_wait_ms_per_read"] = ratio(fg.get("read_ch_wait_ns", 0) / 1_000_000.0, fg_reads)
    row["fg_read_ch_bg_wait_ms_per_read"] = ratio(fg.get("read_ch_bg_wait_ns", 0) / 1_000_000.0, fg_reads)
    row["fg_read_pcie_wait_ms_per_read"] = ratio(fg.get("read_pcie_wait_ns", 0) / 1_000_000.0, fg_reads)
...
def columns() -> list[str]:
    ...
    derived = [
        ...
        "fg_read_die_bg_wait_ms_per_read",
        "fg_read_ch_wait_ms_per_read",
        "fg_read_ch_bg_wait_ms_per_read",
        "fg_read_pcie_wait_ms_per_read",
...
AGGREGATE_METRICS = (
    ...
    "read_p999_ms",
    "write_iops",
    "fg_read_req_latency_avg_ns",
    "fg_read_req_latency_p99_ns",
    "fg_read_req_latency_p999_ns",
    "fg_read_die_wait_ms_per_read",
    "fg_read_die_bg_wait_ms_per_read",
    "fg_read_ch_wait_ms_per_read",
    "fg_read_ch_bg_wait_ms_per_read",
    "fg_read_pcie_wait_ms_per_read",
    "fg_read_priority_yields",
    "fg_read_priority_forced_progress_runs",
    "fg_maint_v2_backlog_max",
    "post_maint_v2_backlog_tasks",
    "post_bg_repromote_backlog",
    "post_bg_qlc_closed_backlog",
    "post_bg_slc_gc_backlog",
)
...
def aggregate_rows(rows: list[dict[str, str]]) -> list[dict[str, str]]:
    ...
    for row in rows:
        key = (row["variant"], row["dist"], row["ratio"], row["bg_timing"])
        groups.setdefault(key, []).append(row)

    out: list[dict[str, str]] = []
    for (variant, dist, ratio_name, bg_timing), group in sorted(groups.items()):
        trials = sorted({row.get("trial", "") for row in group if row.get("trial", "")})
        item = {
            "variant": variant,
            "dist": dist,
            "ratio": ratio_name,
            ...
            "runs": str(len(group)),
            "trials": ",".join(trials),
            "repeat_status": "pass" if len(group) >= 3 else "insufficient",
        }
        for metric in AGGREGATE_METRICS:
            ...
            for row in group:
                ...
                try:
                    ...
                    continue
            item[f"{metric}_n"] = str(len(values))
            item[f"{metric}_median"] = fmt_float(statistics.median(values), 6) if values else ""
            item[f"{metric}_min"] = fmt_float(min(values), 6) if values else ""
            item[f"{metric}_max"] = fmt_float(max(values), 6) if values else ""
        out.append(item)
    return out


def aggregate_columns() -> list[str]:
    base = ["variant", "dist", "ratio", "bg_timing", "runs", "trials", "repeat_status"]
    metrics = []
    for metric in AGGREGATE_METRICS:
        metrics.extend(
            [
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--aggregate-output",
        type=Path,
        help="Write per-variant medians and min/max ranges across repetitions.",
    )
...
def main() -> int:
    ...
    if args.aggregate_output:
        ...
        with args.aggregate_output.open("w", newline="") as aggregate_file:
            writer = csv.DictWriter(
                aggregate_file,
        ...
        print(
            f"[fio-summary] wrote {len(aggregate)} aggregate rows to {args.aggregate_output}"
        )
    return 0
