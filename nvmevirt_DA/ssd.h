...
struct ssdparams {
	...
	/* QLC 延迟参数 - 基于图片参考值 */
	int qlc_pg_4kb_rd_lat[4];  /* QLC 4KB 读延迟 */
	int qlc_pg_rd_lat[4];      /* QLC 页读延迟 */
	int qlc_pg_wr_lat;                           /* QLC 页写延迟 (比 SLC 慢约4倍) */
	int qlc_blk_er_lat;                          /* QLC 块擦除延迟 (比 SLC 慢约3倍) */
	
	/* SLC 到 QLC 迁移延迟 */
	int migration_lat;                           /* 迁移操作额外延迟 */
