...
static unsigned long long nvmev_map_residency_report_hits = 1000000ULL;
module_param_named(map_residency_report_hits, nvmev_map_residency_report_hits, ullong, 0644);
MODULE_PARM_DESC(map_residency_report_hits,
		 "Print mapping DRAM/flash residency every N map cache hits; 0 disables");
...
#endif
/* Variant: DFTL-style 8MiB page-level CMT baseline with LRU. */
#define NVMEV_MAP_CACHE_POLICY_NAME "baseline_lru"
#define NVMEV_MAP_CACHE_HOTCOLD 0
#define NVMEV_MAP_CACHE_SLC_DRAM 0
#define NVMEV_MAP_CACHE_NO_QLC_CMT 0
...
static uint64_t map_cache_dram_entries(struct conv_ftl *conv_ftl)
{
	...
	total_entries = conv_ftl->ssd->sp.tt_pgs;
	entries = (uint64_t)READ_ONCE(mc->used_entries) *
		NVMEV_MAP_CACHE_ENTRIES_PER_SLOT;

	return entries > total_entries ? total_entries : entries;
}

static void map_cache_maybe_report_residency(struct conv_ftl *conv_ftl,
					     uint64_t hits)
{
	struct nvmev_map_cache *mc;
	struct nvmev_map_residency_event *event;
	uint64_t total_entries;
	uint64_t dram_entries;
	uint64_t dram_bp;
	uint64_t flash_bp;
	uint64_t rem;
	unsigned long flags;

	if (!nvmev_map_residency_report_hits || !hits ||
	    !conv_ftl || !conv_ftl->ssd)
		return;

	div64_u64_rem(hits, nvmev_map_residency_report_hits, &rem);
	if (rem)
		return;
	...
	mc = &conv_ftl->map_cache;
	if (!mc->residency_events)
		return;

	dram_entries = map_cache_dram_entries(conv_ftl);
	dram_bp = div64_u64(dram_entries * 10000ULL, total_entries);
	if (dram_bp > 10000ULL)
		dram_bp = 10000ULL;
	flash_bp = 10000ULL - dram_bp;

	spin_lock_irqsave(&mc->lock, flags);
	event = &mc->residency_events[mc->residency_event_head];
	event->seq = ++mc->residency_event_seq;
	event->hits = hits;
	event->dram_bp = (uint32_t)dram_bp;
	event->flash_bp = (uint32_t)flash_bp;
	mc->residency_event_head =
		(mc->residency_event_head + 1) % NVMEV_MAP_RESIDENCY_LOG_ENTRIES;
	if (mc->residency_event_count < NVMEV_MAP_RESIDENCY_LOG_ENTRIES)
		mc->residency_event_count++;
	spin_unlock_irqrestore(&mc->lock, flags);
}

static uint64_t map_cache_record_hit(struct nvmev_map_cache *mc)
{
	return (uint64_t)atomic64_inc_return(&mc->hits);
}

static void map_cache_init_residency_log(struct nvmev_map_cache *mc)
{
	if (!mc)
		return;

	mc->residency_events = vzalloc(sizeof(*mc->residency_events) *
				       NVMEV_MAP_RESIDENCY_LOG_ENTRIES);
	if (!mc->residency_events) {
		NVMEV_WARN("Failed to allocate map residency CSV log; disabled\n");
		return;
	}

	mc->residency_event_head = 0;
	mc->residency_event_count = 0;
	mc->residency_event_seq = 0;
}
...
static void map_cache_decay_locked(struct conv_ftl *conv_ftl)
{
	...
	seq = mc->access_seq;
	div_u64_rem(seq, (uint32_t)NVMEV_MAP_HEAT_DECAY_INTERVAL, &rem);
	if (!mc->hotcold || !seq || rem != 0)
		return;

	for (i = 0; i < mc->nr_entries; i++)
		mc->entries[i].heat >>= 1;
	atomic64_inc(&mc->heat_decays);
}

static void map_cache_touch_locked(struct conv_ftl *conv_ftl,
				   struct nvmev_map_cache_entry *entry)
{
	struct nvmev_map_cache *mc = &conv_ftl->map_cache;

	mc->access_seq++;
	if (mc->hotcold && entry->heat < U16_MAX)
		entry->heat++;
	map_cache_decay_locked(conv_ftl);
	list_move(&entry->lru_node, &mc->lru);
}

static struct nvmev_map_cache_entry *
map_cache_pick_lru_locked(struct nvmev_map_cache *mc)
{
	if (list_empty(&mc->lru))
		return NULL;
	return list_last_entry(&mc->lru, struct nvmev_map_cache_entry, lru_node);
}

static struct nvmev_map_cache_entry *
map_cache_pick_hotcold_locked(struct nvmev_map_cache *mc)
{
	struct nvmev_map_cache_entry *entry;
	struct nvmev_map_cache_entry *victim = NULL;
	uint32_t scanned = 0;
	uint16_t best_heat = U16_MAX;

	if (list_empty(&mc->lru))
		return NULL;

	list_for_each_entry_reverse(entry, &mc->lru, lru_node) {
		if (entry->heat < best_heat) {
			best_heat = entry->heat;
			victim = entry;
		}
		scanned++;
		if (scanned >= NVMEV_MAP_EVICT_SCAN)
			break;
	}

	return victim ? victim : map_cache_pick_lru_locked(mc);
}

static struct nvmev_map_cache_entry *
map_cache_get_slot_locked(struct conv_ftl *conv_ftl, uint64_t *nsecs)
{
	struct nvmev_map_cache *mc = &conv_ftl->map_cache;
	struct nvmev_map_cache_entry *entry;
	uint32_t i;
	...
	entry = mc->hotcold ? map_cache_pick_hotcold_locked(mc) :
			      map_cache_pick_lru_locked(mc);
	if (!entry)
		return NULL;
...
static void map_cache_fill_locked(struct conv_ftl *conv_ftl,
				  struct nvmev_map_cache_entry *entry,
				  uint64_t tpage_id)
{
	struct nvmev_map_cache *mc = &conv_ftl->map_cache;
	...
	if (mc->flash_entries) {
		for (i = 0; i < NVMEV_MAP_CACHE_ENTRIES_PER_SLOT; i++) {
			...
			if (idx < conv_ftl->ssd->sp.tt_pgs)
				entry->entries[i] = mc->flash_entries[idx];
			else
				entry->entries[i].ppa = UNMAPPED_PPA;
		}
	} else {
		do {
			seq = read_seqbegin(&conv_ftl->maptbl_lock);
	...
	if (was_valid)
		...
		list_add(&entry->lru_node, &mc->lru);
		mc->used_entries++;
	}
}

static noinline struct ppa get_maptbl_ent_cached(struct conv_ftl *conv_ftl,
						 uint64_t lpn,
						 uint64_t *nsecs)
{
	struct nvmev_map_cache *mc;
	...
	if (!conv_ftl || !conv_ftl->map_cache.initialized)
...
#if NVMEV_MAP_CACHE_SLC_DRAM
	{
		...
		if (mapped_ppa(&backing) && valid_ppa(conv_ftl, &backing) &&
			...
			hits = map_cache_record_hit(mc);
			atomic64_inc(&mc->slc_dram_hits);
			map_cache_maybe_report_residency(conv_ftl, hits);
			return backing;
		}
	}
#endif
				 ...
				 const struct ppa *ppa)
{
	...
	if (!conv_ftl || !ppa || !conv_ftl->map_cache.initialized ||
	    lpn >= conv_ftl->ssd->sp.tt_pgs)
		return;
...
static int init_map_cache(struct conv_ftl *conv_ftl, bool hotcold)
{
	struct nvmev_map_cache *mc;
	uint32_t i;
	uint64_t idx;
	...
	if (map_flash_store_init(conv_ftl) != 0) {
		...
		vfree(mc->flash_entries);
		memset(mc, 0, sizeof(*mc));
		NVMEV_ERROR("Failed to allocate flash-resident map metadata\n");
		return -ENOMEM;
	}
	...
	map_cache_init_residency_log(mc);
	mc->initialized = true;
...
static void remove_map_cache(struct conv_ftl *conv_ftl)
{
	struct nvmev_map_cache *mc;
...
static int map_cache_stats_show(struct seq_file *m, void *v)
{
	struct conv_ftl *conv_ftl = m->private;
	struct nvmev_map_cache *mc;
	uint64_t lookups, hits, misses;
	uint64_t metadata_read_ns, metadata_write_ns;
	...
	mc = &conv_ftl->map_cache;
	lookups = atomic64_read(&mc->lookups);
	hits = atomic64_read(&mc->hits);
	misses = atomic64_read(&mc->misses);
	metadata_read_ns = atomic64_read(&mc->metadata_read_ns);
	metadata_write_ns = atomic64_read(&mc->metadata_write_ns);
	...
	seq_printf(m, "cached_mapping_entries %llu\n",
		   (unsigned long long)((uint64_t)mc->nr_entries *
					NVMEV_MAP_CACHE_ENTRIES_PER_SLOT));
	seq_printf(m, "used_mapping_entries %llu\n",
		   (unsigned long long)((uint64_t)mc->used_entries *
					NVMEV_MAP_CACHE_ENTRIES_PER_SLOT));
	...
	seq_printf(m, "slc_dram_map_bytes %llu\n",
		   (unsigned long long)(NVMEV_MAP_CACHE_SLC_DRAM ?
					total_slc_pages(conv_ftl) *
					NVMEV_MAP_ENTRY_BYTES : 0ULL));
	...
	seq_printf(m, "metadata_total_ns %llu\n",
		   (unsigned long long)(metadata_read_ns + metadata_write_ns));
	seq_printf(m, "evictions %llu\n",
		   (unsigned long long)atomic64_read(&mc->evictions));
	seq_printf(m, "dirty_evictions %llu\n",
		   (unsigned long long)atomic64_read(&mc->dirty_evictions));
	seq_printf(m, "heat_decays %llu\n",
		   (unsigned long long)atomic64_read(&mc->heat_decays));
	seq_printf(m, "map_residency_report_hits %llu\n",
		   (unsigned long long)nvmev_map_residency_report_hits);
	seq_printf(m, "map_residency_events %u\n",
		   READ_ONCE(mc->residency_event_count));
	return 0;
}

static int map_cache_stats_open(struct inode *inode, struct file *file)
{
	return single_open(file, map_cache_stats_show, inode->i_private);
}

static const struct file_operations map_cache_stats_fops = {
	.owner = THIS_MODULE,
	.open = map_cache_stats_open,
	.read = seq_read,
	.llseek = seq_lseek,
	.release = single_release,
};

static int map_residency_csv_show(struct seq_file *m, void *v)
{
	struct conv_ftl *conv_ftl = m->private;
	struct nvmev_map_cache *mc;
	struct nvmev_map_residency_event event;
	...
	(void)v;
	seq_puts(m, "seq,hits,dram_pct,flash_pct\n");
	if (!conv_ftl)
		return 0;
	...
	count = READ_ONCE(mc->residency_event_count);
	if (count > NVMEV_MAP_RESIDENCY_LOG_ENTRIES)
		count = NVMEV_MAP_RESIDENCY_LOG_ENTRIES;
	head = READ_ONCE(mc->residency_event_head);
	first = (head + NVMEV_MAP_RESIDENCY_LOG_ENTRIES - count) %
		NVMEV_MAP_RESIDENCY_LOG_ENTRIES;

	for (i = 0; i < count; i++) {
		idx = (first + i) % NVMEV_MAP_RESIDENCY_LOG_ENTRIES;
		event = mc->residency_events[idx];
		if (!event.seq)
			continue;
		seq_printf(m, "%llu,%llu,%u.%02u,%u.%02u\n",
			   (unsigned long long)event.seq,
			   (unsigned long long)event.hits,
			   event.dram_bp / 100U, event.dram_bp % 100U,
			   event.flash_bp / 100U, event.flash_bp % 100U);
	}

	return 0;
}

static int map_residency_csv_open(struct inode *inode, struct file *file)
{
	return single_open(file, map_residency_csv_show, inode->i_private);
}

static const struct file_operations map_residency_csv_fops = {
	.owner = THIS_MODULE,
	.open = map_residency_csv_open,
	.read = seq_read,
	.llseek = seq_lseek,
	.release = single_release,
};
...
static void conv_init_ftl(struct conv_ftl *conv_ftl, struct convparams *cpp, struct ssd *ssd)
{
	...
	conv_ftl->debug_map_cache_stats = NULL;
	conv_ftl->debug_map_residency_csv = NULL;
	...
	if (init_map_cache(conv_ftl, !!NVMEV_MAP_CACHE_HOTCOLD) != 0)
		NVMEV_WARN("map CMT disabled; falling back to full in-memory maptbl\n");
	...
	/* 直接初始化水位线（无后台线程） */
	{
		...
		conv_ftl->slc_high_watermark =
			pages_to_lines_ceil(tmp, conv_ftl->slc_pgs_per_blk);
		...
		conv_ftl->qlc_gc_free_thres_high =
			pages_to_lines_ceil(tmp, conv_ftl->qlc_pgs_per_blk);
	...
	nvmev_debugfs_init_instance(conv_ftl);
	{
		...
		conv_ftl->debug_map_cache_stats =
			debugfs_create_file("map_cache_stats", 0440, parent,
					    conv_ftl, &map_cache_stats_fops);
		conv_ftl->debug_map_residency_csv =
			debugfs_create_file("map_residency_csv", 0440, parent,
					    conv_ftl, &map_residency_csv_fops);
...
static void conv_remove_ftl(struct conv_ftl *conv_ftl)
{
	...
	if (conv_ftl->debug_dir) {
		...
		if (conv_ftl->debug_map_cache_stats) {
			debugfs_remove(conv_ftl->debug_map_cache_stats);
			conv_ftl->debug_map_cache_stats = NULL;
		}
		if (conv_ftl->debug_map_residency_csv) {
			debugfs_remove(conv_ftl->debug_map_residency_csv);
			conv_ftl->debug_map_residency_csv = NULL;
		}
...
static bool conv_read(struct nvmev_ns *ns, struct nvmev_request *req, struct nvmev_result *ret)
{
	...
	for (i = 0; (i < nr_parts) && (start_lpn <= end_lpn); i++, start_lpn++) {
		...
		prev_ppa = get_maptbl_ent_cached(conv_ftl, start_lpn / nr_parts, &srd.stime);
		...
		for (lpn = start_lpn; lpn <= end_lpn; lpn += nr_parts) {
			...
			cur_ppa = get_maptbl_ent_cached(conv_ftl, local_lpn, &srd.stime);
			...
			if (xfer_size > 0) {
				...
				if (prev_lpn == INVALID_LPN) {
					...
					refreshed = get_maptbl_ent_cached(conv_ftl, prev_lpn, &srd.stime);
